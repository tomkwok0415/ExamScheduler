...
class ExamTimeSlot:
    def __init__(self, init_exam_duration, break_duration, start_time, name):
        self.start_time = start_time
        self.name = name
        self.break_duration = break_duration
        self.session_duration, self.session_num, self.break_num, self.total_duration = self.calculate_exam_schedule(total_exam_time=init_exam_duration, break_duration=break_duration)
    ...
    def calculate_exam_schedule(self, total_exam_time, break_duration):
        # Step 1: Multiply the exam time by 1.25
        adjusted_exam_time = total_exam_time * 1.25

        sessions = int(adjusted_exam_time // 45)
        session_duration = adjusted_exam_time / sessions
        breaks = sessions - 1
